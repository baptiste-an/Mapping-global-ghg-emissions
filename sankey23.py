...
def junction_5_to_6(data_sankey, region, data, node_dict, color_dict):
    ...
    if test.sum() < 0:
        CFCtoCFCk -= test
        if (RoWCFCtoCFCk + test).abs().sum() > (RoWCFCtoCFCk + test).sum():
            RoWCFCtoCFCk = (RoWCFCtoCFCk + test).abs() * (RoWCFCtoCFCk + test).sum() / (RoWCFCtoCFCk + test).abs().sum()
        else:
            RoWCFCtoCFCk += test
        # CFCtoRoWCFCk += test, aucune raison de faire ça ?
        # CFCtoCFCk -= ((CFCtoRoWCFCk + test).abs()-(CFCtoRoWCFCk + test))/2

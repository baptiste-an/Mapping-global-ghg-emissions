...
def data_Sankey_old(year, region, extension):
...
def data_Sankey(year, region):
    ...
    data["5. cbaK"] = (
        ...
        + data.rename(index=Dict5).index.get_level_values(level="LY name")
    ...
    data["6. cons"] = data.rename(index=Dict6).index.get_level_values(
        level="region cons"
    ...
    data["7. exp"] = data.index.get_level_values(level="region cons")
            ...
            for node in node_list:
                ...
                if node_dict[node] in source_data.index:
                    ...
                    if node in [
                        ...
                        nodes["position"].loc[node] = "7. exp"
                    else:
                        nodes["position"].loc[node] = "6. cons"
            nodes.loc["Net capital formation "]["position"] = "5. cbaK"
...
def fig_sankey(year, region):
    ...
    n = len(nodes.reset_index().set_index("position").loc["6. cons"])

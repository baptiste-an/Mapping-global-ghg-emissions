    ...
    try:
        ...
        try:
            df = (
                ...
                .drop(region, axis=1)
                .sum(axis=1)
            ) - data.xs("CFC<(Lk-L)Y", level="LY name").xs(region, level="region cons").groupby(
                level="sector prod"
            ).sum()[
                "value"
            ]
...
def imports_reexported(data_sankey, region, node_dict, data, color_dict):
    ...
    df = (
        data_sankey.set_index(["source", "target", "color"])["value"]
        .loc[node_dict["RoW - CFC"]]
        .loc[node_dict["RoW - CFCk"]]
        + data_sankey.set_index(["source", "target", "color"])["value"]
        .loc[node_dict["CFC"]]
        .loc[node_dict["RoW - CFCk"]]
        - data_sankey.set_index(["source", "target", "color"])["value"]
        .loc[node_dict["RoW - CFCk"]]
        .loc[node_dict["Exports"]]
    )

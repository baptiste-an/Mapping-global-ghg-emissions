...
def imports_reexported(data_sankey, region, node_dict, data, color_dict):
    ...
    #  RoW - CFCk to CFC imports re-exported
    try:
        df = (
            data["value"]
    ...
    try:
        df = (
            pd.DataFrame(
                data["value"].xs("infRegFromRoW", level="LY name").groupby(level="sector prod").sum(), index=ind
            ).fillna(0)
            + pd.DataFrame(
                data.xs("CFC>(Lk-L)Y", level="LY name")
...
data["value"].xs("infRegFromRoW", level="LY name").sum()


data["value"].xs("CFC>(Lk-L)Y", level="LY name").xs(region, level="region cons").drop(
    region, level="region prod"
).groupby(level="sector prod").sum().sum()
# imports CFC ré exportés

data.xs("CFC>(Lk-L)Y", level="LY name").drop(region, level="region cons").groupby(level="sector prod").sum()[
    "value"
].sum()
# capital RoW venant de reg, pas utilisé en RoW

data.xs("CFC<(Lk-L)Y", level="LY name").xs(region, level="region cons").groupby(level="sector prod").sum()[
    "value"
].sum()
# CFC à l'étranger utilisé en CFCk reg => flux validé : RoW CFC to CFCk

data.xs("CFC<(Lk-L)Y", level="LY name").xs(region, level="region cons").drop(region, level="region prod").groupby(
    level="sector prod"
).sum()["value"].sum()
# CFC à l'étranger utilisé en CFCk reg => flux validé : RoW CFC to CFCk

...
def test():
    df = SLY["LY CFC"].unstack(level="sector cons").sum(axis=1) - (
        SLY[["LkY Government", "LkY Households", "LkY NCF", "LkY NPISHS"]]
        .unstack(level="sector cons")
        .sum(axis=1)
        - SLY[["LY Government", "LY Households", "LY NCF", "LY NPISHS"]]
        .unstack(level="sector cons")
        .sum(axis=1)
    )
    sup = pd.DataFrame(
    ...
    pd.concat([SLY.unstack(level="sector cons"), sup, inf], axis=1)
...
def fig_sankey(year, region):
    ...
    nodes["label"].loc["Consumption of fixed capital"] = ""
    nodes["label"].loc["RoW - Consumption of fixed capital"] = ""
    # nodes["x"].loc[
    #     "Clothing",
    #     "Education",
    #     "Food",
...
def data_Sankey(year, region):
    ...
    data = data.replace(
        dict(
            {"sup": "CFC", "inf": "RoW - CFC", "RoW - sup": np.nan, "RoW - inf": np.nan}
        )
    )
    # 5.jonction
    data["6. cbaK"] = (

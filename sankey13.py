...
def SLY():
    ...
    conc_sec_cons = pd.read_excel(
        "concordance.xlsx", sheet_name="sector cons", index_col=[0, 1]
    )
...
def test():
    ...
    (L.mul(Y.sum(axis=1), axis=1).sum(axis=1) * S_imp).loc["RU"].sum() == D_pba.loc[
        "RU"
    ].sum()
    # pba verification
...
def data_SankeyOK(year, region):
    ...
    SLY["LY GCF"] = (
        SLY["LY CFC"].fillna(0)
        + SLY["LY NCF sup"].fillna(0)
        - SLY["LY NCF inf"].fillna(0)
    )
    ...
    try:
        ...
        supRegFromAll["LY name"] = "supRegFromAll"
        supRegFromAll = supRegFromAll.reset_index().set_index(data.index.names)
        data = pd.concat(
            [
                data,
                supRegFromAll,
    ...
    try:
        ...
        MinusInfRoWFromAll["LY name"] = "MinusInfRoWFromAll"
        MinusInfRoWFromAll = MinusInfRoWFromAll.reset_index().set_index(
            data.index.names
        )
        data = pd.concat(
            [
    ...
    data.loc[
        [i for i in data.index if i[4] == "LY NCF sup" and i[5] != region], ["7. cons"]
    ] = "RoW - Net capital formation "
    data.loc[
        [i for i in data.index if i[4] == "(Lk-L)Y NCF sup" and i[5] != region],
        ["7. cons"],
    ] = "RoW - Net capital formation "
    ...
    data.loc[
        [i for i in data.index if i[4] in right_index_partial and i[5] != region],
        ["8. exp"],
    ] = data.loc[
        [i for i in data.index if i[4] in right_index_partial and i[5] != region]
    ].index.get_level_values(
...
def data_Sankey(year, region):
    ...
    data.loc[[i for i in data.index if i[4] in right_index], ["7. cbaK"]] = (
        data.loc[[i for i in data.index if i[4] in right_index]]
        .rename(index=DictExp)
        .index.get_level_values(level="region cons")
        + data.loc[[i for i in data.index if i[4] in right_index]]
    ...
    data.loc[[i for i in data.index if i[4] in right_index], ["8. cons"]] = data.loc[
        [i for i in data.index if i[4] in right_index]
    ].rename(index=DictRoW).index.get_level_values(level="region cons") + data.loc[
        [i for i in data.index if i[4] in right_index]
    ].index.get_level_values(
    ...
    df = (
        data[["5. endo", "7. cbaK", "value"]]
        .replace(node_dict)
        .dropna()
        .rename(columns={"5. endo": "source", "7. cbaK": "target"})
    )
    df["color label"] = df.index.get_level_values(level="sector prod")
    df["position"] = "5. endo"
    data_sankey = pd.concat([data_sankey, df])
...
# reprendre à NCF, pourquoi gros delta RoW CFC, RoW CFCk Russie
...
def data_households(region, data):
    # Add households direct
...
def data_STV(data_sankey, data, color_dict):
...
def junction_4_to_5(data_sankey, region, data):
...
def junction_5_to_6(data_sankey, region, data):
    data_sankey2 = data_sankey
...
def data_Sankey(year, region):
    ...
    data = data_households(region, data)
    ...
    data_sankey = pd.DataFrame()
    data_sankey = data_STV(data_sankey, data, color_dict)

    # junctions
    data_sankey = junction_4_to_5(data_sankey, region, data)
    data_sankey = junction_5_to_6(data_sankey, region, data)
...
def Nodes():
    ...
    nodes["x"].loc[["LY NCF inf", "RoW - LY NCF inf"]] = 0.38
